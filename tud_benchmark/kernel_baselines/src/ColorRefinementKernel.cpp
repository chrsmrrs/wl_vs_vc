...
namespace ColorRefinement {
    ...
    uint
    ColorRefinementKernel::compute_gram_matrix_count(const uint num_iterations, const bool use_labels, const bool use_edge_labels, const bool compute_gram, const bool wloa) {
        vector<ColorCounter> color_counters;
        color_counters.reserve(m_graph_database.size());

        // Compute labels for each graph in graph database.
        for (Graph &graph: m_graph_database) {
            color_counters.push_back(compute_colors_count(graph, num_iterations, use_labels, use_edge_labels));
        }
    ...
    ColorCounter ColorRefinementKernel::compute_colors(const Graph &g, const uint num_iterations, bool use_labels,
        ...
        ColorCounter ColorRefinementKernel::compute_colors_count(const Graph &g, const uint num_iterations, bool use_labels,
                                                       bool use_edge_labels) {
        size_t num_nodes = g.get_num_nodes();
