...
uint compute_wl_1_sparse_count(string ds, int num_iterations, bool use_labels, bool use_edge_labels) {
    GraphDatabase gdb = AuxiliaryMethods::read_graph_txt_file(ds);
    gdb.erase(gdb.begin() + 0);
    ...
    return wl.compute_gram_matrix_count(num_iterations, use_labels, use_edge_labels, false, false);;
}
...
PYBIND11_MODULE(kernel_baselines, m) {
    ...
    m.def("compute_wl_1_sparse_count", &compute_wl_1_sparse_count);
